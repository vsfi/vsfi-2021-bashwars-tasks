...
class MagicProvides(BaseProvider):
    ...
    def s_number(self):
        if random.randint(1,5) > 3:
            ...
            while True:
                number = "+7%s%i" % (random.choice(oper_numbers), random.randint(1000000, 9999999))
...
_target = User('Grzegorz', 'Brzęczyszczykiewicz', '+79375559798', fake.address(), fake.age(), fake.gender(), fake.email())
_flag = "0PA_CHIRIK!"
...
def get_database():
    ...
    for i in range(random.randint(1000,1200)):
        try:
            user = User(fake.s_name(), fake.s_surname(), fake.s_number(), fake.address(), fake.age(), fake.gender(), fake.email())
...
@app.route('/')
def root():
    response = get_JSON_response({"error":"Ты чо тут забыл, мальчик?"}, 400)
    return response
...
def yasdelal():
    ...
    if not parameter:
        response = get_JSON_response({"error":"'post_parameter' not found"}, 400)
        return response

    if _target.number != parameter:
        response = get_JSON_response({"error":"Слыш, ты нам чо втюхиваешь?"}, 400)
        return response
